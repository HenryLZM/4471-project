...
cp = torch.load('./checkpoints/iter28800.tar')
...
def train_with_sketch(g, d_sketch, sketches, g_opt, d_opt, num, iteration, silence, device='cuda', printevery=10):
    ...
    for i in range(iteration):
       ...
       # print(D_G_z2, label, loss_g)
        ...
        if i % 2000 == 0:
            ToPILImage()(vutils.make_grid(fake, nrow=1, normalize=True)).save(f'./exp/sketch_guide/fake_imgs_{i+50000}.png')
            ToPILImage()(vutils.make_grid(fake_sketches, nrow=1,normalize=True)).save(f'./exp/sketch_guide/fake_sketches_{i+50000}.png')
...
train_with_sketch(g, d_sketch, sketch, g_opt, d_opt, 5, 50000, 0.5)